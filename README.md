...
The app-server protocol snapshot in `packages/codex-driver/src/generated` was generated by Codex CLI `0.147.0`. Startup rejects a different CLI series rather than guessing at protocol compatibility and records the configured model provider's namespace-tool capability. PulseCortex-managed sessions use Codex's paginated history and single-agent tool set; independently created TUI sessions retain their own tool configuration.
