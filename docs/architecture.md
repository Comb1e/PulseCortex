...
Metadata retention defaults to 30 days. Command logs retain at most seven days and 100 MB in total, deleting oldest files first. Readable daemon events are stored below `logs/YYYY-MM-DD/`, with one `HH-NNNN.log` UTF-8 chunk per local hour and a fixed 10 MiB chunk ceiling. Records are never split; an oversized record occupies its own chunk, and a restart continues the latest non-full chunk for the current hour. Daily log retention applies age and the shared byte ceiling to complete date folders, always preserving the active day's folder and ignoring legacy flat files or unrelated entries. The daemon applies both log retention policies at startup and daily.
