...
import { CODEX_PROTOCOL_SNAPSHOT_SERIES, codexEnvironment, detectCodexVersion, resolveCodexInvocation, SUPPORTED_CODEX_CLI_REQUIREMENT } from "@pulsecortex/codex-driver";
...
program.command("diagnose").description("Check Codex, database, pairing, projects, credentials, and delivery queue").action(async () => {
  ...
  try {
    const codex = await detectCodexVersion(config.codexExecutable);
    const detail = !codex.compatible ? `${codex.version} (unsupported; requires ${SUPPORTED_CODEX_CLI_REQUIREMENT})`
      : codex.newerThanSnapshot ? `${codex.version} (newer than ${CODEX_PROTOCOL_SNAPSHOT_SERIES}.x snapshot; daemon verifies app-server compatibility)` : codex.version;
    checks.push({ name: "Codex", ok: codex.compatible, detail });
  }
