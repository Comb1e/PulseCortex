...
The app-server protocol snapshot in `packages/codex-driver/src/generated` was generated by Codex CLI `0.147.0`. Startup rejects a different CLI series rather than guessing at protocol compatibility and records the configured model provider's namespace-tool capability. PulseCortex-managed sessions use Codex's paginated history, single-agent tool set, and configured named permission profile (default `:workspace`); independently created TUI sessions retain their own permission and tool configuration.
...
The supervised daemon may have a different PATH, home directory, and operating-system token from a terminal-launched Codex process. Set `codexExecutable` in `config.json` when Codex is not on the service PATH, and run `pnpm pulsectl diagnose` from the same installed environment to inspect the executable, Codex home, temporary directory, and managed permission profile.
...
| `/instructions` | Show Codex's built-in slash commands, then choose an instruction preset using a Feishu card; creates a session when none is selected |
