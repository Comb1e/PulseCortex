...
When a Codex session reports that terminal or tool calls are unavailable, inspect the current daily log. An `execution environment disconnected` entry means the local Code Mode tool host is unavailable; PulseCortex blocks new turns until Codex reports it ready and fails an active turn instead of letting it continue without tools. If the environment remains connected but the model never receives an `exec` tool, verify the configured model provider. A custom Responses API proxy must preserve custom tool definitions and tool-call items; PulseCortex cannot reconstruct tool schemas removed by an upstream provider.
