...
`pnpm start` displays daemon events as readable timestamped entries with structured fields on indented lines. In an interactive terminal, mutable Feishu card entries keep their complete structured content in a live footer. Before an ordinary log is printed, the daemon clears the reachable footer, prints the log once, commits resolved entries, and redraws active entries at the bottom. Repeated message IDs replace their footer entry without replaying intervening logs. Exact duplicate fingerprints are suppressed after an entry is resolved. If a resize or a footer taller than the terminal makes the old footer unreachable, the renderer safely appends the complete new entry instead of attempting to erase terminal scrollback.
